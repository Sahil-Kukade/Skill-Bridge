      ...
      {mobileMenuOpen && (
        <div className="absolute top-14 left-0 z-50 w-full bg-richblack-800 px-6 py-4 md:hidden">
          <ul className="flex flex-col gap-y-4 text-richblack-25">
            ...
            {/* Cart */}
            {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
              <Link to="/dashboard/cart" className="flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
                <AiOutlineShoppingCart className="text-2xl text-richblack-100" />
                {totalItems > 0 && (
                  <span className="ml-1 rounded-full bg-richblack-600 px-2 py-0.5 text-xs font-bold text-yellow-100">
                    {totalItems}
                  </span>
                )}
              </Link>
            ...
            {token === null ? (
              <>
                <Link to="/login" onClick={() => setMobileMenuOpen(false)}>
                  <button className="w-full rounded-md border border-richblack-700 bg-richblack-700 px-4 py-2 text-left text-richblack-100">
                    Log in
                  </button>
                </Link>
